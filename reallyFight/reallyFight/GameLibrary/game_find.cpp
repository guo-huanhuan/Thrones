		 ...
		 if(abs(x)<man[q].w && abs(y)<man[q].h)//相遇
			{ if(man[i].lb!=man[q].lb) //不同类
				{ man[q].x0=man[q].fx=man[q].xix;//
				  ...
				  man[q].zs=dw[man[q].js][3];man[q].zd=1;//开打	
				  man[i].zs=3;				 man[i].zd=1;//开打	
				  if(man[i].p==man[i].m1-2) sndPlaySound("砍1.wav",SND_ASYNC);//声音
				  if(man[q].p==man[q].m1-8) sndPlaySound("羊.wav",SND_ASYNC);//声音
//				  if(man[i].p==man[i].m1-8) sndPlaySound("羊.wav",SND_ASYNC);//声音
				  break;
				 }
				/*else//是同类，随机移动
